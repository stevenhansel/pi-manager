...
impl Sandbox {
    ...
    fn create_profile_manifest(&self, name: &str, json: &str) {
        let path = self.home.join(".pi-manager").join("profiles").join(format!("{}.json", name));
...
fn test_create_empty_profile() {
    ...
    let manifest = s.home().join(".pi-manager").join("profiles").join("work.json");
    assert!(manifest.exists(), "Profile 'work' should exist at {}", manifest.display());
}
...
fn test_create_from_another_profile() {
    ...
    s.pim().arg("create").arg("work").arg("--from").arg("base").assert().success();

    let manifest_path = s.home().join(".pi-manager").join("profiles").join("work.json");
    let content = fs::read_to_string(&manifest_path).unwrap();
    assert!(content.contains("rtk"), "manifest should contain 'rtk', got: {}", content);
}
...
fn test_create_from_nonexistent_profile_fails() {
    let s = Sandbox::new();
    s.pim()
        .arg("create").arg("work").arg("--from").arg("nonexistent")
        .assert()
        .failure()
...
fn test_list_with_profiles() {
    ...
    let output = s.pim().arg("list").assert().success().get_output().stdout.clone();
...
fn test_use_with_selections() {
    ...
    fs::write(
        s.home().join(".pi-manager").join("pool").join("extensions").join("rtk.ts"),
        "// extension",
    )
    .unwrap();

    s.create_profile_manifest("work", r#"{"select":{"extensions":["rtk.ts"],"skills":[]}}"#);
    s.pim().arg("use").arg("work").assert().success();
    s.assert_active_profile("work");
...
fn test_set_default_nonexistent_fails() {
    let s = Sandbox::new();
    s.pim().arg("set-default").arg("nonexistent").assert().failure();
}
...
fn test_delete_force_removes_profile() {
    let s = Sandbox::new();
    s.pim().arg("create").arg("work").assert().success();
    s.pim().arg("delete").arg("work").arg("--force").assert().success();
...
fn test_delete_nonexistent_fails() {
    let s = Sandbox::new();
    s.pim().arg("delete").arg("nonexistent").arg("--force").assert().failure();
}
...
fn test_migrate_converts_old_profile() {
    ...
    let manifest = s.home().join(".pi-manager").join("profiles").join("legacy.json");
    assert!(manifest.exists(), "manifest should exist after migration");

    // Check pool has the extension
    assert!(
        s.home().join(".pi-manager").join("pool").join("extensions").join("rtk.ts").exists(),
        "extension should be in pool"
    );

    // Check data has auth
    assert!(
        s.home().join(".pi-manager").join("data").join("legacy").join("auth.json").exists(),
        "auth should be in data dir"
    );

    // Check old directory is gone
    assert!(
        !s.home().join(".pi-manager").join("profiles").join("legacy").exists(),
...
fn test_create_then_list_then_use_then_delete_workflow() {
    ...
    // List should show it
    let list_out = s.pim().arg("list").assert().success().get_output().stdout.clone();
    ...
    // Delete it
    s.pim().arg("delete").arg("work").arg("--force").assert().success();
    assert!(!s.home().join(".pi-manager").join("profiles").join("work.json").exists());
}
